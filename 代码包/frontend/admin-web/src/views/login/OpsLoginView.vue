    ...
    <div class="login-card">
      ...
      <div class="login-brand">
        ...
        <div class="brand-content">
          ...
          <h1 class="brand-title">智能垃圾分类监管<br/>与数据分析系统</h1>
          ...
          <div class="brand-features">
            ...
            <div class="feature-item"><span class="feature-icon">🔧</span><span>数据分析</span></div>
          </div>
        </div>
      </div>
      ...
      <div class="login-form-area">
        ...
        <div class="form-footer">AI智能运维管理平台</div>
      </div>
    </div>
    <div class="login-footer">&copy; 2026 智能垃圾分类监管与数据分析系统 v1.0</div>
  </div>
</template>
...
onMounted(() => {
  const saved = localStorage.getItem('garbage_admin_remember_user')
  if (saved) {
    form.username = saved
...
async function handleLogin() {
  ...
  try {
    ...
    if (rememberMe.value) {
      localStorage.setItem('garbage_admin_remember_user', form.username)
    } else {
      localStorage.removeItem('garbage_admin_remember_user')
    }
...
.login-wrapper {
  ...
  background: linear-gradient(135deg, #1a1208 0%, #2c1e10 40%, #1a0e04 100%);
  overflow: hidden;
}
...
.login-brand {
  width: 420px;
  flex-shrink: 0;
  background: linear-gradient(160deg, #e65100 0%, #bf360c 30%, #8d2a00 70%, #5d1a00 100%);
  display: flex;
  align-items: center;
  justify-content: center;
  overflow: hidden;
...
.brand-overlay {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 20% 80%, rgba(255, 152, 0, 0.2) 0%, transparent 50%),
    radial-gradient(circle at 70% 20%, rgba(64, 158, 255, 0.15) 0%, transparent 50%);
}
...
.captcha-img:hover { border-color: #ff9800; }
...
.login-btn {
  ...
  font-weight: 600;
  letter-spacing: 4px;
  --el-button-bg-color: #ff9800;
  --el-button-border-color: #ff9800;
  --el-button-hover-bg-color: #ffb74d;
  --el-button-hover-border-color: #ffb74d;
}
